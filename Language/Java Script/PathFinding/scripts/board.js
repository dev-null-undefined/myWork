  ...
  getNeighbor(x, y, diagonal = false, crossWall = false) {
    ...
    if (diagonal) {
      if (x < this.width - 1 && y > 0) {
        if (
          crossWall ||
          (this.getCell(x, y - 1).type !== Cell.cellTypes.Wall && this.getCell(x + 1, y).type !== Cell.cellTypes.Wall)
        ) {
      ...
      if (x > 0 && y > 0) {
        if (
          crossWall ||
          (this.getCell(x - 1, y).type !== Cell.cellTypes.Wall && this.getCell(x, y - 1).type !== Cell.cellTypes.Wall)
        ) {
          neibrs.push(this.getCell(x - 1, y - 1))
        }
      }
