...
writer_m = cv2.VideoWriter(conf["outPutFile"]+"withMovement-"+str(fileCounter)+".avi",cv2.VideoWriter_fourcc(*'XVID'), float(conf["fps"]),tuple(450,tuple(conf["resolution"])[1]*450/tuple(conf["resolution"])[0]))
newWriter=True
...
if SaveAll:
	...
	saveAllWriter_m = cv2.VideoWriter(conf["saveAllFile"]+"-withMovement.avi",cv2.VideoWriter_fourcc(*'XVID'), float(conf["fps"]),tuple(conf["resolution"]))
...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	if framesAfter > 0:
		writer_m.write(frame)
	if SaveAll:
		saveAllWriter_m.write(frame)
