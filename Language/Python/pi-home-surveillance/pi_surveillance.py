...
fileCounter = 0
writer = cv2.VideoWriter(conf["outPutFile"]+str(fileCounter)+".avi",
                         cv2.VideoWriter_fourcc(*'XVID'), float(conf["fps"]), tuple(conf["resolution"]))
newWriter = True
...
saveAllWriter = None
if SaveAll:
    saveAllWriter = cv2.VideoWriter(conf["saveAllFile"]+str(fileCounter)+".avi",
                                    cv2.VideoWriter_fourcc(*'XVID'), float(conf["fps"]), tuple(conf["resolution"]))
...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if framesAfter > 0:
        ...
        writer.release()
        fileCounter += 1
        writer = cv2.VideoWriter(conf["outPutFile"]+str(fileCounter)+".avi", cv2.VideoWriter_fourcc(
            *'XVID'), float(conf["fps"]), tuple(conf["resolution"]))
        newWriter = True
